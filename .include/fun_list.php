...
function print_table($dir, $list, $allow) {	// print table of files
	global $gperm_handler, $perm_name, $groups, $module_id, $xoopsModuleConfig;
	...
	// Actions
		...
		if(get_is_editable($dir, $item)) {
			if ($gperm_handler->checkRight($perm_name, PERM_EDITFILE, $groups, $module_id)) {
...
function list_dir($dir) {			// list directory contents
	//@@PEM@@
	global $gperm_handler, $perm_name, $groups, $module_id, $xoopsModuleConfig, $xoopsUser;
	// 
	...
	echo "<IMG border=\"0\" width=\"16\" height=\"16\" src=\"_img/_up.gif\" ";
	echo "ALT=\"".$GLOBALS["messages"]["uplink"]."\" TITLE=\"".$GLOBALS["messages"]["uplink"]."\"></A></TD>\n";
	// HOME DIR
	echo "<TD><A HREF=\"".make_link("list",NULL,NULL)."\">";
	echo "<IMG border=\"0\" width=\"16\" height=\"16\" src=\"_img/_home.gif\" ";
	...
	echo " src=\"_img/_refresh.gif\" ALT=\"".$GLOBALS["messages"]["reloadlink"];
	echo "\" TITLE=\"".$GLOBALS["messages"]["reloadlink"]."\"></A></TD>\n";
	if ($gperm_handler->checkRight($perm_name, PERM_SEARCH, $groups, $module_id))
	{		
		// SEARCH
	...
	if($allow) {
		// COPY
		if ($gperm_handler->checkRight($perm_name, PERM_COPY, $groups, $module_id))
		{
		...
		// MOVE
		if ($gperm_handler->checkRight($perm_name, PERM_MOVE, $groups, $module_id))
		{
		...
		// DELETE
		if ($gperm_handler->checkRight($perm_name, PERM_DELETE, $groups, $module_id))
		{		
		...
		// UPLOAD
		if ($gperm_handler->checkRight($perm_name, PERM_UPLOAD, $groups, $module_id))
		{				
	...
	// Create File / Dir
	if ($gperm_handler->checkRight($perm_name, PERM_CREATE, $groups, $module_id))
	{			
		if($allow) {
	...
	echo "</A></B></TD>\n<TD WIDTH=\"10%\" class=\"headxplorer\"><B>";
	...
	if ($xoopsModuleConfig['showsize']) {
		...
		echo "</A></B></TD>\n<TD WIDTH=\"16%\" class=\"headxplorer\"><B>";
	}	
	if ($xoopsModuleConfig['showtype']) {
		...
		echo "</A></B></TD>\n<TD WIDTH=\"14%\" class=\"headxplorer\"><B>";
	}	
	if ($xoopsModuleConfig['showmodify']) {
	...
	if ($xoopsModuleConfig['showperm']) {
		echo "</A></B></TD><TD WIDTH=\"8%\" class=\"headxplorer\"><B>".$GLOBALS["messages"]["permheader"]."</B>\n";
	}
	echo "</TD><TD WIDTH=\"6%\" class=\"headxplorer\"><B>".$GLOBALS["messages"]["actionheader"]."</B></TD></TR>\n";
	...
	echo "<TR><TD colspan=\"7\"><HR></TD></TR><TR>\n<TD class=\"headxplorer\"></TD>";
	echo "<TD class=\"headxplorer\">".$num_items." ".$GLOBALS["messages"]["miscitems"]." (";
	...
	echo "<TD class=\"headxplorer\">".parse_file_size($tot_file_size)."</TD>\n";
	for($i=0;$i<4;++$i) echo"<TD class=\"headxplorer\"></TD>";
